...
st.set_page_config(page_title="Pelan Beast v64", layout="wide")
...
st.markdown("<div class='beast-card' style='text-align:center;'><h1 style='color:#d4af37;'>🏗️ Pelan Engineering Beast v64</h1><p class='gold'>الموسوعة الشاملة (زلازل - خزانات - خرسانة) | م. بيلان عبد الكريم</p></div>", unsafe_allow_html=True)
...
# -- حسابات الخرسانة (جوائز، بلاطات، أعمدة، أساسات) --
...
with c2:
    ...
    st.markdown(f"<div style='border:2px dashed #d4af37; padding:20px; text-align:center; border-radius:15px; background:rgba(0,0,0,0.2);'><h2 style='color:#50c878;'>{main_steel}</h2><p class='gold'>↑ توصيف الوحش الهندسي ↑</p><hr style='border-color:#d4af37;'><p style='color:#aaa;'>الكانات: Φ10 @ 15cm</p></div>", unsafe_allow_html=True)
    if st.button("🛠️ تصدير DXF 🚀"):
        doc = ezdxf.new(setup=True); msp = doc.modelspace()
        msp.add_text(f"PELAN BEAST v64 - {category}", dxfattribs={'height': 5}).set_placement((0, 0))
        buf = io.StringIO(); doc.write(buf)
        st.download_button("📥 تحميل المخطط", buf.getvalue(), "Pelan_Beast.dxf")
    st.markdown("</div>", unsafe_allow_html=True)

st.markdown("<p style='text-align:center; color:#d4af37;'>Pelan Beast v64 | Final Unstoppable Version</p>", unsafe_allow_html=True)
