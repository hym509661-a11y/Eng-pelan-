...
st.set_page_config(page_title="Pelan Beast v72", layout="wide")
...
st.markdown("<div class='card' style='text-align:center;'><h1 style='color:#d4af37;'>🏗️ Pelan Engineering Suite v72</h1><p class='gold'>الموسوعة الشاملة | م. بيلان عبد الكريم | 2026</p></div>", unsafe_allow_html=True)
...
with c2:
    ...
    if st.button("🛠️ تصدير DXF 🚀"):
        ...
        msp.add_text(f"PELAN v72", dxfattribs={'height': 5}).set_placement((0, 0))
        buf = io.StringIO(); doc.write(buf)
        st.download_button("📥 تحميل المخطط", buf.getvalue(), "Pelan_Beast.dxf")
    st.markdown("</div>", unsafe_allow_html=True)

st.markdown("<p style='text-align:center;color:#d4af37'>Pelan Beast v72 | Final Unbreakable Core</p>", unsafe_allow_html=True)
